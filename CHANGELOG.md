...
1. **文件存储位置优化**
   - 修改 GitHub 存储路径为 `codespace-manager/keepalive_tasks.json`
   - 更好的仓库组织结构
   - GitHub API 自动创建目录，无需手动操作
...
3. **Commit 信息优化**
   - 新格式：`[Auto] Update keepalive tasks (N active) - timestamp`
   - 显示当前活动任务数量
   - 更清晰的自动提交标识
   - 便于追踪和审计
...
4. **增强的日志和诊断** 🆕
   - 添加详细的调试日志输出
   - 新增 `test_github_storage.py` 诊断工具
   - 可以快速定位配置问题
   - 提供详细的错误信息
...
- 新增 `TROUBLESHOOTING.md` - 完整的故障排查指南 ⭐
- 新增 `test_github_storage.py` - GitHub 存储诊断工具
- 更新 `CLOUD_STORAGE_SETUP.md` - 添加并发控制、目录自动创建说明
- 更新 `README.md` - 添加故障排查链接
- 添加常见问题解答：多人使用、目录创建、文件位置
...
1. **保活任务自动保存**
   ...
   - 本地：保存到 `keepalive_tasks.json` 文件
   - 云端：使用 GitHub API 存储（完全免费）

2. **☁️ Streamlit Cloud 支持**
   - 新增 `github_storage.py` - GitHub API 存储后端
   - 自动后端切换：本地用文件，云端用 GitHub
   - 配置简单，使用现有 GitHub 账户
   - 数据存储在你的私有仓库，完全安全
...
- 关闭浏览器不影响保活任务
- 重启服务后保活任务继续有效
- Streamlit Cloud 上完全可靠的持久化

### 📚 文档更新

- 新增 `CLOUD_STORAGE_SETUP.md` - 云存储配置完整指南
- 更新 `KEEPALIVE_GUIDE.md` - 添加持久化说明和新的检查频率
- 更新 `SECURITY.md` - 添加数据持久化说明
- 更新 `README.md` - 强调持久化特性
- 更新 `.streamlit/secrets.toml.example` - 添加 GitHub 存储配置
